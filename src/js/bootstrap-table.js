 ...
 * --- example ---
 * 
 *	$(attr).paging({			//@attr 元素id
 *		pageNo:pageNo,			//当前页
 * 		totalPage: totalPage,	//总页数
 *   	callback: initData		//回调
 *  })
 *
 * --- example ---
 *
...
(function ($, window, document, undefined) {
	...
	function Paging(element, options) { //定义分页类

		this.element = element; //当前元素id

		$(this.element).addClass('page_div') //给当前元素添加样式class

		this.options = { //传入形参
			pageNo: options.pageNo || 1, //当前页
			totalPage: options.totalPage, //总页数
			totalSize: options.totalSize, //总条数
			isSearch: options.isSearch, //是否添加搜索框  默认 添加
		...
		this.init(); //根据形参初始化分页html和css代码
	}

	Paging.prototype = { //对Paging的实例对象添加公共的属性和方法

		constructor: Paging,

		init: function () {
			styCount++;
			if (styCount <= 1) $("head").append("<style pop-animation=true>" + genStyle(this) + "</style>");
			this.creatHtml();
			this.bindEvent();
		},
		creatHtml: function () {
			...
			var me = this,
				content = "",
				current = me.options.pageNo,
				total = me.options.totalPage,
				totalNum = me.options.totalSize,
				isSearch = me.options.isSearch,
				selectedIdx = me.options.selectedIdx
			fir = "", des = "";
			if (total <= 1 || !total) return $(this.element).html(""); //如果当前总页数小于1 删除分页
			if (current == 1) fir = 'curNodrop'
			content = "<a href='javascript:;' class='prePage " + fir + "'>上一页</a>";
			//总页数大于6时候
			if (total > 10) {
				//当前页码小于5时末段显示省略号
				if (current < 5) {
					for (var i = 1; i < 6; i++) {
						if (current == i) {
							content += "<a href='javascript:;' class='current'>" + i + "</a>";
						} else {
					...
					if (current < total - 3) {
						...
						for (var i = 0; i < 5; i++) {
							idx++;
							if (current == idx) content += "<a href='javascript:;' class='current'>" + idx + "</a>";
							else content += "<a href='javascript:;'>" + idx + "</a>";
						}
						content += " ... ";
						content += "<a href='javascript:;'>" + total + "</a>";
						...
						for (var i = total - 5; i <= total; i++) {
							...
							if (current == i) content += "<a href='javascript:;' class='current'>" + i + "</a>";

							else content += "<a href='javascript:;'>" + i + "</a>";
						}
					}
				}
				//页面总数小于6的时候
			...
			if (current == total) des = 'curNodrop'
			content += "<a href='javascript:;' class='nextPage " + des + "'>下一页</a>";
			// content += "<a href='javascript:;' class=\"lastPage\">尾页</a>";
			var selectOpt = "",newSetVal = ""
			// log(selectedIdx)
			if(selectedIdx){
				for (var index = 1; index < 5; index++) {
					var cIdx = index*10
					if (selectedIdx == cIdx) {
						selectOpt += '<option value="' + cIdx + '" selected >' + cIdx + '</option>'
					}else{
						selectOpt += '<option value="' + cIdx + '">' + cIdx + '</option>'
					}
					
				}

				newSetVal = '<span style="padding: 0 10px;"> 每页' +
					'<select class="linitPage" style="height:100%;margin:0 3px;border-radius: 5px;border: 1px solid #dce0e0 !important;" >' +
						selectOpt + 
					'</select>条 </span> ' 
			}
			// log(selectOpt)
			if (isSearch !== false) {
				content += '<div style="display: inline-block;margin-left: 20px"> ' +
								newSetVal +
							'<input type="text" style="width:50px;border-radius: 5px;padding-left: 3px;height: 100%;"/><a href="javascript:;" class="jump"> 转到 </a></div> ';
			}
		...
		bindEvent: function () {
			...
			me.element.on('click', 'a', function () {
				...
				// log(me.options.callback)

				var cls = $(this).attr("class") ? $(this).attr("class") : '';

				if (cls.indexOf("prePage") !== -1) { //点击上一页

					if (me.options.pageNo == 1) return false

					else me.options.pageNo = +me.options.pageNo - 1;

				} else if (cls.indexOf("nextPage") !== -1) { //点击下一页

					if (me.options.pageNo == me.options.totalPage) return false

					else me.options.pageNo = +me.options.pageNo + 1;

				} else if (cls.indexOf('jump') !== -1) { //点击跳页

					var pno = $(this).prev('input').val()

					if (!pno || isNaN(pno)) return $(this).prev('input').val("")

					else if (pno >= me.options.totalPage) pno = me.options.totalPage

					else if (pno <= 1) pno = 1

					me.options.pageNo = pno

				} else if (me.options.pageNo == +num) return false //点击current页

				else me.options.pageNo = +num;

				me.creatHtml();

				if (me.options.callback) me.options.callback(me.options.pageNo,me.options.selectedIdx);

			});
			me.element.off('change', 'select');
			me.element.on("change", 'select', function (e) {
				var tar = e.target
				me.options.selectedIdx = $(tar).val()
				if (me.options.callback) me.options.callback(1,me.options.selectedIdx);
			})
		}
	};
...
function genStyle(a) {
	var ies = ".table tbody tr:hover .dss{ background:#F5F5F5  !important;}.table tbody tr:hover{ background:#F5F5F5  !important;} "
	/*
	 * 外面盒子样式---自己定义
	 */
	var css = ies
