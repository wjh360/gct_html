$(function () {
    ...
    var count = 0;
    var curProjectId;
    var applyId = storage("contractId")  //工程支付申请id
    $http({
        ...
        success: function (r) {
            ...
            getDownSelect('contractList', r.data.constructContract.contractName, r.data.contractList, 'id', 'contractName', ['secondParty']);
            $(".contractList").attr("index", r.data.constructContract.id)
            $(".secondParty").html(r.data.constructContract.secondParty)
            noTedData = r.data.taskList
            selectedData = r.data.list
            $.each(selectedData, function (index, eleVal) {
                selectedId.push(eleVal.id)
            })
            // resetTable()
            if (!r.data.appropriate) return false
            curProjectId = r.data.appropriate.projectId
            $(".money").val(r.data.appropriate.amount)
            $(".moneyBil").val(r.data.appropriate.ratio)
            resetTable(1)
            $(".unpaid").html(r.data.appropriate.nowAmount)//待支付金额
        }
    })
    ...
    function resetTable(ads) {
        ...
        ads || $(".unpaid").html(count.toFixed(2) || '0.00')
    }
    //点击未选列表 选择工单 
    ...
    $('body').on('click', '.postBtn', function () {
        ...
        $.popConfirm({
            ...
            'confirm': function () {
                $http({
                    url: ' /gct-web/finance/appropriateUpdate',
                    data: obj,
                    success: function (r) {
