...
$(function () {
    //请求公司 结构
    $http({
        url: '/gct-web/auditProcess/addEcho',
        data: {
            auditItem: auditItem
        },
        success: function (r) {
    ...
    function initDepMents(a, b, c) {
        if (b == 'apply_comply_tree') {  //申请人
            curClickCompanyId = c.id
        } else {
            curAuditClickCompanyId = c.id   //审核人
        }
        $http({
            ...
            success: function (r) {
                if (b == 'apply_comply_tree') { //生成申请人部门树
                ...
                if (b == 'depMent_tree') {
                    ...
                    $.each(getData, function (i, val) {
                        if (val) {
                            ...
                            str += '<span class="overf_E applyPersonCheck allsort" style="display:' + dis + '"  index="' + val.id + '" >' + val.trueName + '</span>'
                        }
                    })
    ...
    $("body").on("click", '.checkAllPerson', function () {
        ...
        if ($(this).hasClass("checkBoxTrue")) {
            ...
            $(this).addClass('checkBox').removeClass("checkBoxTrue").parents('.apply_checkAllPersonP').hide()
        ...
        if (isAudit) {
            $.each(auditUserList, function (i, val) {
                ...
                str += '<p class="clearfix auditsel">' +
                    ...
                    '<span class="sel_contnet overf_E">' + val.name + ' ( ' +
                    val.companyName + 
                    dep
                    + ' ) </span>' +
            ...
            var dataList = isAudit ? auditPersonDataList : applyPersonDataList
            dataList = applyPersonDataList
    ...
    $("body").on("click", '.com_cancel', function () {
        ...
        if ($(this).attr("name") == 'sel_person') {
            ...
            allUserList.indexOf(curId) == -1 || allUserList.splice(allUserList.indexOf(curId), 1)
            applyUserList.indexOf(curId) == -1 || applyUserList.splice(applyUserList.indexOf(curId), 1)
            creatSelectedContent_list('apply_SelectedContent_list')
    ...
    $("body").on('click', '.postBtn', function () {
        ...
        $.popConfirm({
            ...
            'confirm': function () {
                $http({
                    url: '/gct-web/auditProcess/add',
                    data: {
                        auditItem: auditItem,                       //：1001 事项编码
                        apply: applyUserList.join(),                       //：“1，2，3，4，5” 申请人 //用户ID以逗号分隔
                        audit: audit.join()                      //：“2，3，4，5”审核人 //用户ID以逗号分隔，按照顺序给 
                    },
                    success: function (r) {
                        submitSuccess('提交成功', goBack)
                    }
                })
            }
        })
