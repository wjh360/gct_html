...
function reformData(data) {
    ...
    if (data.statistic) {
        ...
        var scDta = data.statisticData || {}
        arr1 = [obj], arr2 = [obj]
        for (var k in arrObj) {
            arrObj[k]['amount'] = sc[k] || '0.00'
            arrObj[k]['t_name'] = k
            if (arrObj[k]['isEdit'] && !arrObj[k]['noDD']) {
                arrObj[k]['amountTit'] = scDta[k] || '0.00'
            }
            // log(k, sc[k])
...
$(function () {
    $("body").on("click", "#tableTwoTable1 .confirmTablePn", function () {
        var data = {
            budgetId: budgetId,
            flag: tab_rowData['flag']
        }
        var oldVal = data[tab_rowData['t_name']]
        data[tab_rowData['t_name']] = $(this).siblings("input").val()
        if (oldVal == data[tab_rowData['t_name']]) return submitSuccess("操作成功")
        $http({
            url: "/gct-web/tableThreeNail/tableTwoUpdate",
            data: data,
            success: function (r) {
                submitSuccess('操作成功')
                getEchoTableTwo()
    ...
    $("body").on("click", "#tableTwoTable2 .confirmTablePn", function () {
        var data = {
            budgetId: budgetId,
            flag: tab_rowData['flag']
        }
